...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, IconButton, Link, MenuItem, Paper, Select, Switch, TableHead, TextField, Tooltip, Typography } from '@material-ui/core';
          ...
          {({ values, isSubmitting }) => {
            return (
              <Form autoComplete="off">
                <DialogTitle id="form-dialog-title">{addMode ? t('Add lease type') : t('Update lease type')}</DialogTitle>
                <DialogContent>
                  {addMode && (
                    <DialogContentText>
                      ...
                      <Grid item xs={6}>
                        <SelectField
                          ...
                          values={timeRanges.map(timeRange => (
                            { id: timeRange, label: t(timeRange), value: timeRange }
                          ))}
                          disabled={values.usedByTenants}
                        />
                      </Grid>
                ...
                <DialogActions>
                  ...
                  <SubmitButton
                    label={!isSubmitting ? (addMode ? t('Add') : t('Update')) : t('Submitting')}
                  />
                </DialogActions>
              </Form>
            );
...
const LeaseTypes = withTranslation()(observer(({ t, setError }) => {
  const store = useContext(StoreContext);
  ...
  const onRemoveLeaseType = useCallback(async leaseType => {
    ...
    if (status !== 200) {
      switch (status) {
        case 422:
          return setError(t('One lease is used by tenants. It cannot be removed.'));
        case 403:
          return setError(t('You are not allowed to update the lease.'))
        default:
          return setError(t('Something went wrong'));
    ...
    <FormSection label={t('Manage lease types')}>
      ...
      <Paper variant="outlined" square>
        <Table aria-label="lease type table">
          <TableHead>
            <TableRow>
              <TableCell><Typography>{t('Contract')}</Typography></TableCell>
              <TableCell><Typography>{t('Description')}</Typography></TableCell>
              <TableCell><Typography>{t('Number of terms')}</Typography></TableCell>
              <TableCell align="center"><Typography>{t('Active')}</Typography></TableCell>
              <TableCell></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {React.Children.toArray((store.leaseType.items || [])
              .map(leaseType => {
                return (
                  <TableRow size="small">
                    <TableCell>
                    ...
                    <TableCell>
                      {leaseType.system || !leaseType.numberOfTerms ? (
                        ...
                        <Typography noWrap>{t('{{numberOfTerms}} {{timeRange}}', { numberOfTerms: leaseType.numberOfTerms, timeRange: leaseType.timeRange })}</Typography>
                      )}
                    </TableCell>
                    <TableCell align="center">
                      <Switch
                        ...
                        onChange={evt => onLeaseTypeChange(evt.target.checked, leaseType)}
                      />
                    </TableCell>
                    <TableCell>
                      {!leaseType.system && (
                        <Box whiteSpace="nowrap">
                          ...
                          <Tooltip title={leaseType.usedByTenants ? t('Lease currently used in tenant contracts') : ''}>
                            <span>
                              <IconButton
                                aria-label="delete"
                                onClick={() => setLeaseTypeToRemove(leaseType)}
        ...
        >
          ...
          <Box py={2}>
            <Typography variant="h6" align="center">{leaseTypeToRemove.name}</Typography>
          </Box>
        </ConfirmDialog>
      </Paper>
    </FormSection>
