...
import { CheckboxField, FormTextField, SelectField, SubmitButton } from '../Form';
...
const NewTenantDialog = withTranslation()(({ t, open, setOpen, onConfirm }) => {
  ...
  const _onSubmit = async tenantPart => {
    ...
    if (tenantPart.isCopyFrom) {
      const {
        _id, reference, name, manager, terminated, beginDate, endDate, terminationDate, properties, discount, guaranty,
        ...originalTenant
      } = toJS(store.tenant.items.find(({ _id }) => tenantPart.copyFrom === _id ));
  ...
  const tenants = store.tenant.items
    ...
    .filter((tenant, index, tenants) => {
      return tenants.findIndex(currentTenant => (currentTenant.name === tenant.name)) === index;
    })
                  ...
                  { !!tenants.length && (
                    <>
                      <CheckboxField
                        ...
                        aria-label={t('Copy information from an existing tenant')}
                      />
                      {values.isCopyFrom && (
                        <SelectField
                          name="copyFrom"
