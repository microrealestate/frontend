...
const TenantForm = withTranslation()(({ t, readOnly, onSubmit }) => {
  ...
  const initialValues = {
    ...
    contacts: store.tenant.selected?.contacts?.length ? store.tenant.selected.contacts.map(({ contact, email, phone, phone1, phone2 }) => ({
      contact,
      email,
      phone1: phone1 || phone,
      phone2: phone2 || ''
          ...
          <Form autoComplete="off">
            <FormSection label={t('Tenant information')}>
              ...
              <RadioFieldGroup
                ...
                name="isCompany"
              >
                <RadioField value="false" label={t('A personal account')} disabled={readOnly} />
                <RadioField value="true" label={t('A business or an institution')} disabled={readOnly} />
              </RadioFieldGroup>
